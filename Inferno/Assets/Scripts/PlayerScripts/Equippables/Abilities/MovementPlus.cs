...
public class MovementPlus : Ability
{
    ...
    void Start()
    {
        ...
        effects[0].effects.Add("movementSpeed", 1.5f);

        effects[1].idNumber = 1002;
        effects[1].effects = new Dictionary<string, float>();
        effects[1].effects.Add("movementSpeed", 1.5f);

        effects[2].idNumber = 1003;
        effects[2].effects = new Dictionary<string, float>();
        effects[2].effects.Add("dashCooldown", 0.8f);
        effects[2].effects.Add("dashSpeed", 1.0f);
        effects[2].effects.Add("dashTime", 1.0f);
    }
}
